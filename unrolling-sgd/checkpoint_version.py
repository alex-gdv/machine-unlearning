...
for ep in range(args.finetune_epochs):
    for main_idx,(inputs, targets) in enumerate(data_loader):
        ...
        #print('unlearning error is = ', rolling_unl_error)
        #print('curr error is', curr_error)
        #print('time is ', t_num)
        #print('delta weights is', delta_weights)
        #print('sigma rolled is ',(sum(sigma_list[-t_num:])/len(sigma_list[-t_num:])))
        ...
        #print(f"done actual_idx = {actual_idx} Previous error is ={rolling_unl_error_list[-2]} and now its {rolling_unl_error_list[-1]}. curr_error = {curr_error}")
        #if main_idx == 10:
        #    break
ret = {}
